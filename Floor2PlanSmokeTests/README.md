...
```text
https://release-candidate.floor2plan.com/Account/Login
```
...
## Import Configuration system table tests

Regression tests for the **Import Configuration** system table (`ImportConfig`) live under Application Settings → System administration → Tables.

```sh
cp .env.smoke.example .env.smoke.local
# SMOKE_SERVICE_USERNAME=reg user
# SMOKE_SERVICE_PASSWORD=test
# TARGET_URL=https://release-candidate.floor2plan.com/Account/Login

npm run test:system-tables
```

Coverage:

- Opens `/system-administration/tables/ImportConfig` after service login
- Asserts the **Additional config** column is present (`additionalConfig` field from `GetTableInfo`)
- Cancelling inline edit (Escape) does not persist changes
- Selecting another row discards unsaved Additional config edits
- Edit mode is cleared after cancel + row change

The Additional config column tests require that field on the target environment. If the column is not yet deployed, the navigation smoke test still passes and the column assertions list available columns from `GetTableInfo/ImportConfig`.

Optional overrides:

- `SMOKE_ADDITIONAL_CONFIG_COLUMN_FIELD` — defaults to `additionalConfig`
- `SMOKE_ADDITIONAL_CONFIG_COLUMN_TITLE` — regex source, defaults to `additional\s*config`
