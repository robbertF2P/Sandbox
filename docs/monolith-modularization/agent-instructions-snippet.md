...
For **Primavera XER bulk project import** (~9k components, ~250k activities, intermediate `ProjectStructureImportModel` + shared `IProjectImportService`, batched EF upsert): `docs/modularization/primavera-xer-import-service-instructions.md` — run against the **external F2P monolith repo**; SandBox POCs are `reference_only`.
