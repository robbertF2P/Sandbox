...
The agent must run Phases A–G against the **external Floor2Plan monolith repository** — the legacy production application — **not** this SandBox workspace.
...
1. Open the **external monolith repository** as the workspace root in your IDE.
...
5. Cross-reference `docs/monolith-modularization/analysis-instructions.md` for entry-point IDs (EP-###) and bounded context names.
...
known_external_systems:          # seed list; agent validates and expands
  - name: SAP
...
```text
EXTERNAL INTEGRATIONS — PHASE A ONLY: Discovery sweep.
...
```text
EXTERNAL INTEGRATIONS — PHASE B: Integration catalog for bounded context "<CONTEXT_NAME>".
...
### Reverse-engineering heuristics (tell the agent to apply)
...
### Agent prompt (run once per bounded context)

```text
EXTERNAL INTEGRATIONS — PHASE C: Reverse-engineered story map for "<CONTEXT_NAME>".
...
```text
EXTERNAL INTEGRATIONS — PHASE C ROLLUP: Cross-context story map index.
...
### Classification heuristics (tell the agent to apply)
...
```text
EXTERNAL INTEGRATIONS — PHASE D ONLY: Lead/follow classification.
...
```text
EXTERNAL INTEGRATIONS — PHASE E: Domain language and use-case linkage.
...
### Best practices (instruct the agent to evaluate and apply)
...
```text
EXTERNAL INTEGRATIONS — PHASE F: Target architecture and recommendations.
...
- docs/monolith-modularization/agent-instructions-snippet.md (target topology)
...
```text
EXTERNAL INTEGRATIONS — PHASE G: Gaps and test plan.
...
Align naming with docs/monolith-modularization/analysis-instructions.md Phase 4.
```
...
```text
Using docs/modularization/integrations/contexts/<context>/story-map.md,
...
```text
Trace all code paths involving "<VENDOR>" (e.g. Kronos).
...
```text
Entity type "<ENTITY>" appears led by both F2P and "<VENDOR>".
...
```text
Design integration pack "<pack-name>" for <VENDOR> <domain>.
...
```text
Submodule "<path>" implements client-specific integration.
