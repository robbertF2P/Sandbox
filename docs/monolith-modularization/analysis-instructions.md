# Monolith modularization — analysis instructions

**Purpose:** Guide AI coding assistants (GitHub Copilot, Claude Code, and similar) through systematic analysis of a legacy monolith repository to extract bounded-context use cases and define a characterization test suite that enables safe refactoring into a composed modular architecture.

**Audience:** Engineers leading modularization; any AI assistant used for analysis or implementation in the **external monolith repo**.
...
1. Open the **external monolith repository** as the workspace root in your IDE (VS Code, Visual Studio, JetBrains, etc.).
2. Attach or reference this file (and `agent-instructions-snippet.md`) in your assistant — e.g. `.github/copilot-instructions.md`, `docs/modularization/agent-rules.md`, or pasted into chat.
...
| Phase | Suggested assistant mode | Why |
|-------|--------------------------|-----|
| 0–2 Discovery | Chat + full-repo context | Broad search (`@workspace`, `@codebase`, or equivalent) |
| 3 Use cases | Agent / autonomous or chat with file refs | Deep dive per area |
| 4 Test design | Chat | Structured output |
| 5 Module cut | Agent / autonomous | Multi-file reasoning |
...
known_contexts:                   # optional seed list; agent validates/refines
  - name: Orders
...
**Prompt convention:** The monolith repo must be the **workspace root**. Prefix any prompt below with your tool's full-repository scope when needed (e.g. `@workspace`, `@codebase`, `#codebase`, or attach `agent-rules.md`).
...
```
Use full-repository context (@workspace, @codebase, repo-wide search, or your tool's equivalent).
You are analyzing a legacy monolith for modularization.
...
```
PHASE 1 ONLY: Entry-point catalog.
...
### Discovery heuristics (tell the agent to apply these)
...
```
PHASE 2 ONLY: Bounded context map.
...
```
PHASE 3: Use case extraction for bounded context "<CONTEXT_NAME>".
...
```
PHASE 4: Test suite plan for bounded context "<CONTEXT_NAME>".
...
When an AI assistant moves from analysis to implementation, enforce:
...
```
Find all references from <ContextA> code to <ContextB> types, tables, or namespaces.
...
```
Table <TableName> is used by contexts A and B.
...
```
From use-cases.yaml, rank P0 gaps by: change frequency, bug history, coupling, refactor blocker.
...
```
For context <CONTEXT_NAME>, list UI routes, components, and API calls.
Map each to UC- IDs. Propose Nx library split matching backend context.
```

---

## Anti-patterns (instruct the agent to avoid)
